"""
Analyzes the output of an experiment (by looking into the detailed results JSON file) and exports them into a csv file.
"""
...
def read_experiment(xp_name):
    ...
    filter_level = df_results.loc[df_results["experiment_id"] == xp_name][
        "filter_level"
    ].values[0]
    if type(filter_level) != str:  # in case there is no filter there will be a nan
        filter_level = "None"
    lemma_preprocessing = df_results.loc[df_results["experiment_id"] == xp_name][
        "lemma_preprocessing"
    ].values[0]
    meta = {
...
def add_info_to_df(df, question, fiche_info, meta):
    ...
    df.loc[
        ...
        & (df.fiche == get_fiche_name(fiche_info))
        & (df.level == meta["filter_level"]),
        [f"score_{retriever_type}_{lemma_preprocessing}"],
    ] = fiche_info[2]
    df.loc[
        (df.question == question)
        & (df.fiche == get_fiche_name(fiche_info))
        & (df.level == meta["filter_level"]),
        f"position_{retriever_type}_{lemma_preprocessing}",
    ] = fiche_info[1]
    return df
...
list_xp = ["b850", "1ea4", "a0ef", "3a76", "862d", "ba96", "208c", "9e00"]
