...
from src.evaluation.config.elasticsearch_mappings import (ANALYZER_DEFAULT,
                                                          DPR_MAPPING,
                                                          SBERT_MAPPING,
                                                          SPARSE_MAPPING)
...
def load_25k_test_set(test_corpus_path: str):
    ...
    for row in df.iterrows():
        ...
        id = row[1]["id"]
        arbo = {"theme": row[1]["theme"], "dossier": row[1]["dossier"]}
...
def compute_retriever_precision(true_fiches, retrieved_results, weight_position=False):
    ...
    retrieved_doc_names = [
        ...
        for idx, f in enumerate(retrieved_results)
    ]
    for fiche_idx, true_fiche_id in enumerate(true_fiches):
        for retrieved_doc_idx, retrieved_doc in enumerate(retrieved_results):
            ...
            if true_fiche_id in retrieved_doc_id:
                if weight_position:
                    summed_precision += (fiche_idx + 1) / (
                        fiche_idx + retrieved_doc_idx + 1
                    )
                else:
                    summed_precision += 1
                break
...
def single_run(parameters):
    ...
    clean_function = None
    if (
        preprocessing and retriever_type != "bm25"
    ):  # we do not use the preprocessing func if we are using pure ES
...
def load_cached_dict_embeddings(
    knowledge_base_path: Path,
    retriever_type: str,
    cached_dicts_path: Path = Path("./data/dense_dicts/"),
    preprocessing: bool = False,
):
    cached_dicts_name = cached_dicts_path / Path(
        f"{knowledge_base_path.name}_{retriever_type}_{preprocessing}.pkl"
    )
    if cached_dicts_name.exists():
        ...
        try:
            ...
            logger.info(
                f"Could not load dict embeddings {cached_dicts_name}. Error: {str(e)}"
            )
            return
    else:
        return
...
def load_retriever(
    ...
    if not knowledge_base_path.exists():
        raise FileNotFoundError(
            f"Knowledge base folder {knowledge_base_path} does not exist!"
        )
    clean_function = None
    ...
    try:
        ...
        if retriever_type == "bm25":
            ...
            dicts = []
            if use_cache:
                dicts = load_cached_dict_embeddings(
                    knowledge_base_path=Path(knowledge_base_path),
                    retriever_type=retriever_type,
                    preprocessing=preprocessing,
                )
    ...
    for question, meta in test_dataset.items():
        ...
        if filter_level is None:
            retrieved_results = retriever.retrieve(
                query=question, top_k=retriever_top_k
            )
        else:
            ...
            if (
                not filter_value
            ):  # sometimes the value for the filter is not present in the data
                logger.info(f"Fiche(s) {meta['urls']} have no filter data available.")
            else:
                filter_ = {filter_level: [filter_value]}
            retrieved_results = retriever.retrieve(
                query=question, filters=filter_, top_k=retriever_top_k
            )
        pbar.update()

        precision, results_info = compute_retriever_precision(
            true_fiche_ids, retrieved_results, weight_position=weight_position
        )
