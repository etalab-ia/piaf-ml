...
Usage: squad_to_dpr.py <squad_file_path> <dpr_output_path> [options]
...
from haystack.document_store.elasticsearch import \
    ElasticsearchDocumentStore  # keep it here !
...
"""
SQuAD format { version: "Version du dataset" data:[ { title: "Titre de l'article Wikipedia" paragraphs:[ { context:
"Paragraph de l'article" qas:[ { id: "Id du pair question-réponse" question: "Question" answers:[ { "answer_start":
"Position de la réponse" "text": "Réponse" } ], is_impossible: (not in v1) } ] } ] } ] }


DPR format [ { "question": "....", "answers": ["...", "...", "..."], "positive_ctxs": [{ "title": "...", "text": "...."
...
class HaystackDocumentStore:
    def __init__(self, store_type: str = "ElasticsearchDocumentStore", **kwargs):
        if store_type not in ["ElasticsearchDocumentStore", "FAISSDocumentStore"]:
            raise Exception(
                "At the moment we only deal with one of these types:"
...
class HaystackRetriever:
    def __init__(
        self, document_store: BaseDocumentStore, retriever_type: str, **kwargs
    ):
        if retriever_type not in [
            "ElasticsearchRetriever",
            "DensePassageRetriever",
            "EmbeddingRetriever",
        ]:
            raise Exception(
                "Use one of these types: ElasticsearchRetriever",
                "DensePassageRetriever",
                "EmbeddingRetriever",
            )
    ...
    def get_retriever(self):
        return globals()[self._retriever_type](
            document_store=self._document_store, **self._kwargs
        )
...
def create_dpr_training_dataset(
    squad_data: dict, retriever: BaseRetriever, num_hard_negative_ctxs: int = 30
):
    ...
    for idx_article, article in enumerate(tqdm(squad_data, unit="article")):
        ...
        for paragraph in article["paragraphs"]:
            ...
            for question in paragraph["qas"]:
                ...
                hard_negative_ctxs = get_hard_negative_context(
                    retriever=retriever,
                ...
                positive_ctxs = [
                    {"title": f"{article_title}_{i}", "text": c}
...
def split_and_save_dataset(
    iter_dpr: Iterator, dpr_output_path: Path, total_nb_questions: int
):
...
def get_hard_negative_context(
    retriever: BaseRetriever, question: str, answer: str, n_ctxs: int = 30
):
    list_hard_neg_ctxs = []
    ...
    for retrieved_doc in retrieved_docs:
        ...
        list_hard_neg_ctxs.append(
            {"title": retrieved_doc_id, "text": retrieved_doc_text}
        )
...
def main(
    squad_file_path: Path,
    dpr_output_path: Path,
    ...
    squad_impossible_path, squad_data = add_is_impossible(
        load_squad_file(squad_file_path=squad_file_path), json_file_path=squad_file_path
    )

    # 2. Prepare document store
    store_factory = HaystackDocumentStore(
        store_type=document_store_type_config[0], **document_store_type_config[1]
    )
    document_store: BaseDocumentStore = store_factory.get_document_store()

    # 3. Load data into the document store
    document_store.add_eval_data(
        squad_impossible_path.as_posix(),
        doc_index="document",
