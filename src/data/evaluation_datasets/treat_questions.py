"""
Analyzes the 25k mail file from service-public and extract those incoming messages that contain a question (identified
by a question mark)
"""
...
def process_incoming_message(incoming_message):
    """
    :param incoming_message: :return: a dict {'question': length of the question in words} Note that the punctuation if
    counted as a word
    """
    result = {}
...
for l in tqdm(lines[:]):
    ...
    if "<url>" in l:
        ...
        if "url" not in new_line:
            ...
            new_line[
                "url_4"
            ] = temp  # there are sometimes more links but they will be discarded
    if "</qr>" in l:
