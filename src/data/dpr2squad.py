...
Usage:
    dpr2squad.py <dpr_file_path> <squad_output_path> [options]
...
def create_squad_eval_dataset(dpr_file_path: Path):
    ...
    for question in tqdm(dpr_file):
        doc_dict = {}
        paragraph_dict = {"context": question["positive_ctxs"][0]["text"],
                          "qas": [{"answers": [{"answer_start": 0, "text": question["answers"][0]}],
                                   "question": question["question"],
                                   "is_impossible": False,
                                   "id": hashlib.md5(str(question["question"]).encode("utf-8")).hexdigest()[:8]
                                   }]}
